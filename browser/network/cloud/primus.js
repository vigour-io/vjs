...
var EventEmitter = require('eventemitter3') 
  //0.4 kb larger then the one primus uses but ok, now its available troughout the framwwork
  , ua = require('../../ua')
  ...
  primus.on('incoming::open', function opened() {
    ...
    if (primus.buffer.length) {
      for (var i = 0, length = primus.buffer.length; i < length; i++) {
        primus.write(primus.buffer[i]); //dit wordt al eerder gestirngified
      }
...
Primus.prototype.backoff = function backoff(callback, opts) {
  ...
  //
  opts.timeout = opts.attempt !== 1 ? Math.min(Math.round(
...
//
if (
  'object' === typeof JSON && 'function' === typeof JSON.stringify && JSON.stringify(['\u2028\u2029']) === '["\u2028\u2029"]'
) {
...
if (ua.browser === 'firefox' && ua.version < 22) {
  //
...
} else if (ua.browser==='safari' && ua.version < 5.1) {
  //
