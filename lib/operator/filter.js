                ...
                $change: function filterListener(event) {
                  ...
                  if(check) {
                    if(!resultsRef) { // checks out but not in
                      ...
                      results.$push(value, event) // TODO: if sorted use $splice
                      resultsRef = results[results.length-1]
                      delta = { added: [ 0 ] }
                    }
                  } else {
                  ...
                  if(delta) {
                    console.log('there is a change in the filter!')
                    results.$emit('$operatorChain', event, delta)
                  }
                }
              })
