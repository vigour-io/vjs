'use strict'
var Emitter = require('../../emitter')
var Datalayer = require('./datalayer')
var servicesActivated = require('../service')

module.exports = new Emitter({
  properties: {
    services (val) {
      for (let i in val) {
        this.services[i] = val[i]
      }
    }
  },
  define: {
    services: { value: {} },
    emitterKeys: {
      value: {
        error (data, event, bind, key, ignore, dataLayer) {
          dataLayer.eventObject.metaMessage = data
        }
      }
    },
    parseInput (data, event, bind, key, ignore, id) {
      let dataLayer = new Datalayer({
        id: id,
        eventObject: {
          eventType: event.type,
          eventOriginator: event.origin.path[0],
          stamp: event.stamp
        }
      })
      if (this.emitterKeys[key]) {
        this.emitterKeys[key].call(this, data, event, bind, key, id, dataLayer)
      }
      return dataLayer
    },
    emit () {
      let parsed = this.parseInput.apply(this, arguments)
      for (let service in servicesActivated.services) {
        servicesActivated.services[service](parsed)
      }
      Emitter.prototype.emit.apply(this, arguments)
    }
  }
})
