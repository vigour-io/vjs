...
exports.$define = {
  ...
  $exec:function( bind, event ) {
    ...
    if(!this._$parent) {
      ...
      console.warn('emitter without parent: blocked!', this.$key)
      console.log('?!!', this === window.wtf1, this === window.wtf2)
      return
    }
  ...
  if( contextBinds ) {
    ...
    for( var i = contextBinds.length-1; i >= 0; i-- ) {
      ...
      console.log('%cexecute emitter - context', 'padding:4px; background: #333; color: orange'
       ...
       //right here!!!!
      if( bind._$contextLevel && !myContextLevel ) { //this && !myContextLevel fixes the test but bad need to put it back..
