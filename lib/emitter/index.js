...
var Emitter = module.exports = new Base({
  ...
  $define: {
    ...
    $exec:function( bind, event, bound ) {
      ...
      if( emitter.$onPasson ) {
        emitter.$onPasson.each(function(property) {
          execPasson( property, bind, event )
        }, exclude, stamp )
      }
    ...
    $postpone: function( event, bind ) {
      ...
      if( 
        ...
        if(this._$parent._$parent !== bind ) {
          // console.log('push to $bind', bind.$path, 'on:', this._$parent._$parent.$path)
          //this.$parent.$parent
          this.__$bind__.push( bind )
        }
        
        if( !this.hasOwnProperty('$myPostPonedStamp') || this.$myPostPonedStamp !== event.$stamp ) {
          event.$postpone( this )
        }

        if(!bind.hasOwnProperty('$postPonedStamp')) {
          // console.error('create postponed', this.$uid, this.$path)
          bind.$postPonedStamp = {} 
        }
    ...
    $emit:function( event, bind, force, meta ) {
      ...
      // console.group()
      // console.warn( 'EMIT EMITTER:', this._$parent._$parent.$path, '\n\nemit!', bind.$path, 'old:' ,this.$lastStamp, 'new:', event.$stamp  )
      if( this.$lastStamp !== event.$stamp ) {

        // console.log( 'emit! pass', bind.$path )

        if( event.$origin !== bind && !force ) {

          // console.log( 'emit! postpone this one', bind.$path )

          this.$postpone( event, bind )
        } else {

          // if(!force) {
          //   console.error('\n!!!orginator exec!!!', bind.$path, '\n\n')
          // } else {
          //   console.log('exec', force, bind.$path)
          // }
...
function execPasson( property, bind, event ) {
  if( property[2] ) {
      property[0].apply( 
      bind, 
