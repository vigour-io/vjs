...
var Emitter = module.exports = new Base({
  ...
  $define: {
    ...
    $exec:function( bind, event, bound ) {
      ...
      if( !bound && emitter.__$bind__ ) {
        for( var i in emitter.__$bind__ ) {
          if( emitter.__$bind__[i] !== bind ) {
            emitter.$exec( emitter.__$bind__[i], event, true )
          } else {
            console.error( 'dont exec myself twice! -- why am i here again?', bind.$path )
          }
        }
        emitter.__$bind__ = null
      }

      emitter.$isPostponed = null
      bind.$postPonedStamp = null
    ...
    $postpone: function( bind, event ) {
      ...
      if( 
        ...
        if( !bind.hasOwnProperty( '$postPonedStamp' ) || !bind.$postPonedStamp ) {
          bind.$postPonedStamp = {} 
        } 

        bind.$postPonedStamp[this.$uid] = event.$stamp
      }
    },
