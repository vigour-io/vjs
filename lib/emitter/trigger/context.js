  ...
  while (parent) { // !parent._context FIX THIS!
    ...
    if (parentInstances) {
      ...
      for (let i = 0, length = parentInstances.length; i < length; i++) {
        ...
        let target = instance
        console.log('try looper', instance._path, context && context._path, bind._path)
        // console.log( instance instanceof context._Constructor)
        // match op level!
        console.log(Object.getPrototypeOf(instance), context)
        // console.log(Object.getPrototypeOf(context) === instance, Object.getPrototypeOf(instance)._path)
        if (
          !context ||
          instance === context ||
          instance._parent === context //|| // this ofc crappy
          // Object.getPrototypeOf(instance) //=== Object.getPrototypeOf(context)
        ) {
          console.log('FOUND! looper', instance._path, path, bind._path)
          ...
          if (path) {
            ...
            for (let j = pathlength; j >= 0; j--) {
              ...
              target._contextLevel = pathlength - j
          ...
          if (target && target[bind.key] === bind) {
            ...
            bind._contextLevel = path ? path.length + 1 : 1
            console.log('ass ballz', bind.path, instance._path)
            emitter.execInternal(bind, event, data)
            bind._context = null
            bind._contextLevel  = null
          }
