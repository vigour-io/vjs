  ...
  while (parent) { // !parent._context FIX THIS!
    ...
    if (parentInstances) {
      ...
      for (let i = 0, length = parentInstances.length; i < length; i++) {
        ...
        let target = instance
        console.log('looper', instance._path, context && context._path)

        // match op level!

        if (!context || instance === context || instance._parent === context) {
          console.log('FOUND! looper', instance._path, path)
