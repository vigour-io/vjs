...
<<<<<<< HEAD
                ...
                throw new Error('no emitter! WTF' + bind._path.join('.'))
=======
                console.error('no emitter:', bind._path.join('.'))
                return
                // throw new Error('no emitter! WTF' + bind._path.join('.'))
>>>>>>> cfa98cdf9aed72bc09aeff025746410f262c8bf7
              }
              emitter.execInternal(bind, event, data)
              // console.error('emit context!')
      ...
      // become espacialy problematic with many parents
      return
    }
    if (!path) {
