  ...
  while (parent && !parent._context) {
    ...
    if (parentInstances) {
      for (let i = 0, length = parentInstances.length; i < length; i++) {
        // this can also be opmized lets do instances in bind maps -- way better
        // then when specific you can just find your uid
        // in that map we can store paths as well -- also constructing the path smarter lets try
