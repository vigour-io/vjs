...
exports.define = {
  ...
  execInternal (bind, event) {
    ...
    // if(bind.lastStamp && bind.lastStamp === emitter.lastStamp) {
    //   return
    // }
    console.log('execinternal!', bind._path, bind.uid, data, event.stamp)
    // try {
    ...
    // }
    if (emitter.base) {
      console.log('base!', bind._path, bind.uid, data, event.stamp)
      ...
      emitter.base.each(function (property) {
        if (property.lastStamp === event.stamp) {
          ...
          console.log('base!2', property._path)
  ...
  trigger (event) {
    ...
    // more like a lookup or only the last same problem
    // last has to set it? laststamp will not hold circular though!
