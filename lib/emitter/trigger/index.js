    ...
    if (bind) {
      ...
      if (emitter.attach) {
        emitter.attach.each(function (property) {
          // emitter is own instance
          // bind._context ? bind._context._uid :
          // have to do something else -- it just has to go and emit for each derived instance and context of the subsc
          // var stamp = event.stamp + bind._uid
          // if (property.lastStamp === stamp) {
          //
          // } else {
            // property.lastStamp = stamp
            // property.clearContext()
            // extra arg ? over reference may make things faster for subscription
