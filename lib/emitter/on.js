...
function add (emitter, type, val, key, unique, event) {
  ...
  // remove the binds crack after this works

  if (emitter[type][key]) {
    emitter[type].removeProperty(emitter[type][key], key, true)
  }
  // removeKeyFromOtherStores(key, type, emitter) THIS IS SLOW REMOVE!
  // JUST DONT SWITCH LISTENER TYPES LIKE THAT BE NORMAL

  if (event) {
    event.on('close', function () {
      // so much crack spawning from this here unfornutly
