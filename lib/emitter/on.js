...
function parseUnique (emitterType, val, emitter, key, type) {
...
function resolve (emitter, type, key) {
  ...
  if (emitter[type][key]) {
    //      console.log('ok so why are we destorying shit hur?')

    // ok so what goes wrong here -- adds the listener --- adds it too fast???? very strange then decides to remove it again
    //
    // why is it immediatly made ? should not happen ....
    console.log(type, key) // NO WRONG!
...
function add (emitter, type, val, key, unique, event) {
  ...
  if (!emitterType) {
    if (event) { // make trigger rly smart
