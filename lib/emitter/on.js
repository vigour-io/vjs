...
function resolve (emitter, type, key, log) {
  if (log) {
    console.log(type, '??xxx??2222?????', !!emitter.fn)
  }
  ...
  if (emitter[type][key]) {
   ...
   if (log) {
    console.log('??xxx??2222?????', !!emitter.fn, type, emitter)
  }
}
...
function add (emitter, type, val, key, unique, event) {
  var log = false
  var p = emitter._path
  if (p && p[2] === 'currentSeason' && emitter.key === 'data') {
    log = true
    console.log('make new???', !!emitter.fn, key, emitter._path)
  }
  ...
  if (!emitterType) {
    if (event) {
      ...
      emitter.emitting = true
      if (log) {
        console.log('ok!', emitter.uid, !!emitter.fn, val, emitter)
      }
      addInternal(emitter, type, key, val)
      return
    }
