...
exports.define = {
  ...
  set: function (val, event, nocontext) {
    ...
    if (isPlainObj(val) && !(val instanceof Array)) {
      for (var key in val) {
...
function add (emitter, type, val, key, unique, event) {
  ...
  if (!emitterType) {
    ...
    if (isFn) {
      ...
      // e.g. using uid for fieds or something simliar
      // lets make a method for these type of id generators
      if (type === 'base') {
        // console.log('????', key)
