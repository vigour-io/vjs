...
exports.define = {
  ...
  set: function (val, event, nocontext) {
    ...
    if (util.isPlainObj(val) && !(val instanceof Array)) {
      for (var key in val) {
        // use this for the on stuff way better
        // call resolvecontext in here remove the stuff on on
        var propertyDefinition = this._properties && this._properties[key]
        if (propertyDefinition) {
          console.log('a set..', key, event)
          propertyDefinition.call(this, val[key], event, nocontext)
        } else {
          if (isNumberLike(key)) {
            if (!this._id) {
              this._id = Number(key)
...
function add (emitter, type, val, key, unique, event) {
  ...
  if (event) {
    console.warn('hey???', event.stamp, key)
