...
exports.define = {
  ...
  bind (event, bind, data, key) {
    var bound = this.getBind(event, bind, key)
    if (!bound) {
      bound = this.setBind(event, bind, data)
    }
    return bound
  },
  setBind (event, bind, data, noval) {
    ...
    // remove refs -- cnt refs? perhaps or better if isempty(top) if used as ref sets ref
    if (data !== void 0) {
      bound.data = data
    }
    return bound
