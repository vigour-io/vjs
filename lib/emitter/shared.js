...
exports.removeFromListens = function ( listens, emitter ) {
  if(listens) {
    for( var i in listens ) {
      ...
      if( i[0] !== '_' ) {
        //make this better make special type for listens on that does not use contextResolvers
        //this will make eveything very very slow!
        if( listens[i] === emitter ) {                      
          delete listens[i]
          if( listens['_'+i] ) {
            console.error('?does this happen?')
            delete listens['_'+i]
          }
        } 
      } else {
        console.log( '???', i, listens[i]  )
      }
    }
  }
}
