...
function doConditionEvent (bound, emitter, event, val, uid) {
  ...
  // delete emitter.binds[event.stamp]
...
function copyDeep (bound, obj, val, conditionEvent, event, context) {
  ...
  for (var i in bound) {
    console.log(i)
    if (isNumberLike(i)) {
      console.log('CONTEXT'.red.bold, i)
      obj[i] = copyDeep.call(this, bound[i], {}, val, conditionEvent, event, context)
    } else {
      obj[i] = bound[i]
      // if (i === 'chain') {
      //   // let bind = this.setContextChain(bound[i])
      //   // this.triggerInternal(bind, val, conditionEvent, event) //deze moet nu ook executen
      //   // if contexts clear it
      //   // context[0].bind.setContextChain(context)
      //   // only needs ot happen once
      //   // RESTORE!
      // }
    }
  }
