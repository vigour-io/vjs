...
// Removed this, this causes a circular require (youzi)
  ...
  $define: {
    ...
    $execInternal: function ( bind, event ) {
      ...
      if ( this._$input) {
        ...
        event.$condition = progress
        event.$origin = bind

        var ret = this._$input.call(bind, function execEmitter () {
          var inProgress = condition.$inProgress
          if ( inProgress ) {
            var length = inProgress.length
            var i = length - 1
