...
module.exports = function (observable) {
  ...
  observable.define({
    ...
    set: function (val, event, nocontext, escape) {
      if (event === void 0) {
    ...
    addNewProperty: function (key, val, property, event, escape) {
      ...
      if (fireParentEvent && this[key] instanceof Observable) {
        // console.log('???', fireParentEvent, event, property, event.removed)
        // maybe make this optional?
        if( event && event.removed ) {
          console.error('ok i just removed the event...')
        }
        this[key].emit('parent', this, event) // event
      }
    }
  })
}
