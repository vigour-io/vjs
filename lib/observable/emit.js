...
emitQueue: function (type, event, context) {
  if (event) {
    ...
    if (
      ...
      var emitterSettings
      // speed emitterSettings up by making a map based on type key
      // also needs to take into account default settings instead of using properties
      if (
        queue &&
        (!(emitterSettings =
            this.get(['_properties', 'on', 'base', type, 'base'])
          ) || emitterSettings.executeQueue !== false
        )
      ) {
        event.loop()
      }
