"use strict";

var Event = require('../event')
var util = require('../util')
var convertToArray = util.convertToArray

//TODO: file needs a lot of cleaning and splitting up

exports.$inject = [
  require( './instances'),
  require( './instances/emit' )
 ]


//remove the internal exec that should just be a postponed or something
//exec everything the same (allmost true)

exports.$define = {
  emit : function( type, event, meta, extraMeta ) {
    if( event === false ) {
      return
    }

    //console.log('emitting:', event && event.$stamp, this._$path)
    //this mofo has to become unified

    var context = this._$context
    var emitter = this.$on && this.$on[ type ]

    if( emitter ) {
      if( event === void 0 ) {
        event = new Event( this, type )
      }
      this.$emitInstances( emitter, event, context, arguments )
      if( !context ) {
        this.$clearContext()
      }
      // console.log('%c emit this!', 'color:blue', this.$path)
      //this is something we want to eradicate in some way..
      emitInternal( emitter, this, type, event,  meta, extraMeta, arguments )
    }
    this.$emitPostponed( type, event, context )
    return event
  },
  $emitPostponed: function( type, event, context ) {
    // console.log(context)
    if( event
      && event.$type === type
      && ( event.$origin === this
          && event.$context == context //can be null or undefined as well
        )
      && !event.$block
    ) {
      var postponed = event.$postponed
      var flags = this._$flags
      if(
        postponed &&
        flags &&
        ( !flags.$on.$base._$flags[type] ||
          flags.$on.$base._$flags[type].$base.$postponed !== false
        )
      ) {
        emitLoop(event)
        event.$postponed = null
      }
    }
  }
}

function emitInternal( emitter, base, type, event,  meta, extraMeta, args ) {
  if( meta ) {
    //for perf
    if( extraMeta ) {
      args = convertToArray( args )
      args.unshift( event )
      args[1] = this
      args[2] = void 0
      emitter.emit.apply( emitter, args )
    } else {
      emitter.emit( event, base, void 0, meta )
    }
  } else {
    emitter.emit( event, base )
  }
}

function emitLoop(event, p, metaPostponed, m) {
  var postponed = event.$postponed
  if(!p) {
    p = 0
  }
  for( var emitter$ ; (emitter$ = postponed[p]); p++ ) {
    if( emitter$.$meta ){
      if( !metaPostponed ) {
        metaPostponed = []
      }
      metaPostponed.push( emitter$ )
    } else{
      // console.log('%cemit:', 'color:green;font-weight:bold;' , emitter$._$path)
      emitter$.emit( event, void 0, true )
    }
  }

  if( metaPostponed ) {
    var cachedLength = postponed.length
    if( !m ) {
      m = 0
    }
    for( var meta$ ; (meta$ = metaPostponed[m]); m++ ) {
      meta$.emit( event, void 0, true )
    }
    if( cachedLength !== postponed.length ) {
      emitLoop( event, p, metaPostponed, m )
    }
  }
}
