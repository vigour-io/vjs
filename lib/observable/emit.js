...
exports.$define = {
  $emit : function( type, event, meta, extraarg ) {
    ...
    if( emitter ) {
      ...
      if( meta ) {
        ...
        if(extraarg) {
          ...
          console.info(type, '2 - emit it!')
          emitter.$emit( event, this, void 0, meta )
        }
      } else {
        console.info(type, 'emit it!', event.$stamp )
    ...
    if( 
      ...
      !this._$flags.$on.$base._$flags[type] || 
      !this._$flags.$on.$base._$flags[type].$base ||
      this._$flags.$on.$base._$flags[type].$base.$executePostponed !== false
    ) {
      var postponed = event.$postponed
      if( postponed ) {
        //this for can grow while executing thats why there is no length cache
        // event.i = 0 double check if nessecary
