...
var vigourStorage = require('./vigourStorage')
...
module.exports = function (observable, event) {
  ...
  function getFromStorage (data, event) {
    var stored = vigourStorage.getItem(this.storageKey)
    if (stored) {
    ...
    data: {
      storage (data, event) {
        if (data === null && this.storageKey) {
          vigourStorage.removeItem(this.storageKey)
          return
        }
        if (!isPlainObj(data)) {
          ...
          if (typeof data !== 'object') {
            vigourStorage.setItem(this.storageKey, data === false ? '' : data)
          }
        } else {
