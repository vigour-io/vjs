...
module.exports = function onProperty (data, event, emitter, pattern, info, mapValue, mapObj) {
  if (data) {
    ...
    if (added) {
      ...
      for (; i >= 0; i--) {
        let key = added[i]
        if (pattern[key]) {
          if (!found && !getLateral(info) && !pattern._context) {
            pattern = getPattern(this, mapValue, [])
          }
          mapObj.parent = mapValue
          emitter.subscribeField(data, event, this[key], pattern, key, info, mapObj)
          found = true
        }
      }
