...
function walkRefs(target, emitter, pattern, passon, originalTarget, event) {
  ...
  while( target._$val instanceof Base ) {
    ...
    // target.on('$reference', function(){
    //   console.warn('>>>>> yay reference event!')
    // })

    target.on('$change', function(){
      console.warn('>>>> wut only change!')
    })
  ...
  return function missingPropHandler(event, meta) {
    ...
    if(added && added.indexOf(key) !== -1) {
      ...
      if(sub.$val === true) {
        ...
        targetProp.on( '$change', passon )
        // add key to emitter._$meta.added
        addAddedKey( emitter, key )
        emitter.$emit( event, originalTarget )
      } else {
        subscribe( targetProp, emitter, sub, passon, originalTarget, event )
      }
      ...
      for(var i = 0, l = added.length ; i < l ; i++) {
        ...
        if(firstChar !== '_' && firstChar !== '$') {
          ...
          if(sub.$val === true) {
            ...
            console.log('---> hang dat listener!', targetProp._$key)
            targetProp.on( '$change', passon )
...
function addAddedKey( emitter, key ) {
  var meta = emitter._$meta
  if(!meta) {
    emitter._$meta = {
      added: [ key ]
...
function mergeAddedList(emitter, added) {
  var cache = emitter._$meta
  ...
  if(!cache) {
    ...
    cache.added = added.slice()
  } else {
    cache.added.push.apply( cache.added, added )
  }
}
