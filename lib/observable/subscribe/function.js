...
var subscribe = module.exports = function subscribe( target, emitter, event,
  ...
  if( pattern._$val === true ) {
    ...
    if( event === void 0 ){
      event = new Event( originalTarget, emitter.$key )
    }
    ...
    if( key === 'any$' ) { // any$
      ...
      if( patternPart.$val === true ) { // directly
        ...
        target.each(function( targetProp ) {
          ...
          event = subscribe( targetProp, emitter, event, patternPart,
            originalTarget
          )
        })
      }
...
function createRefHandler( emitter, pattern, originalTarget ) {
  return function refHandler( event, oldvalue ) {
    ...
    log.error('********* subscription refListener!',
      '\npath:', this.$path,
      '\noldvalue\n', oldvalue
    )
    ...
    if( targetVal === null ) {
      log.shine('hey the reference itself is removed man, handle that')
      handleRemove( target, emitter, event, pattern, originalTarget )
      ...
      if( !(targetVal instanceof Base) ) {
        ...
        target = walkRefs( target, emitter, pattern, originalTarget, true )
      }
    ...
    if(!emitter._$emitting) {
      // log.shine('should be emitting hurt!')
      var subsmeta = emitter._$meta || (emitter._$meta = {})
      addChangeToMeta( subsmeta, target, originalTarget,
        pattern
      )
      emitter.emit( event, originalTarget )
    }
...
function handleRemove( target, emitter, event, pattern, originalTarget ) {
  log.shine('something has been removed, better do things!', target.$path)

  var pointer = getRemovePointer( target, pattern )

  target = pointer[0]
  pattern = pointer[1]

  log.shine('got pointer', target.$path, pattern)

  var subsmeta = emitter._$meta || (emitter._$meta = {})

  addChangeToMeta( subsmeta, target, originalTarget, pattern )

  if( target._$val !== null ) {
    log.shine('pointer is empty ref')
  } else {
    log.shine('pointer is something removed')
    pattern = pattern.$parent
    target = target.$parent
    target = unWalkRefs( target, pattern )
    log.shine('add missing prop listener to', target.$path)
    addMissingPropListener( target, emitter, pattern, originalTarget )
  }
...
function clean( target, pattern ) {
  ...
  //   '\npattern:', pattern,
  //   '\ntarget.$on.$change', target.$on && target.$on.$change
  // )

  target = cleanRefs( target, pattern )

  if(pattern.$val === true) {
    // log('found endpoint! remove change listener!')
    ...
    pattern.each(function( patternPart, key ) {
      if( key === 'any$' ) {
        // log('oh gosh remove anyListener from target and then',
        //   'clean all properties of target'
        // )

        target.off('$property', { $attach: { $check:
          function( found ){
            return found[2] && found[2][1] === patternPart
          }}
        })

        target.each(function(targetProp){
          clean(targetProp, patternPart)
        })

      } else {
        // log('see if target has property', key, '\n',
        //   'if not, remove missingPropListener (if not already done)\n',
        //   'if so, continue cleaning property', key
        // )
        var targetProp = target[key]
        if( targetProp ) {
          // log('it has',key,'property!')
          clean( targetProp, patternPart )
        } else if( !removedMPL ){
          // log('does not have, has mph?!')
          target.off('$property', { $attach: { $check:
            function( found ) {
              return found[2] && found[2][1] === pattern
            }}
...
function cleanRefs( target, pattern ) {
  var val = target._$val
  while( val instanceof Base ) {
    target.off('$reference', { $attach: { $check: removeCheck } })
    target = val
    val = val._$val
  }
  return target
  ...
  function removeCheck( found ) {
    // console.warn('check for removal!', found[2] && found[2][1] === pattern)
    return found[2] && found[2][1] === pattern
  }
}
  ...
  return function targetHandler(event, remove) {
    ...
    if(remove) {
      if(this === originalTarget) {
        ...
        handleRemove( this, emitter, event, pattern, originalTarget )
        log.error('wext', subsmeta)
        //
        // var removePointer = addRemoveToMeta( subsmeta, this, originalTarget,
        //   pattern
        // )
        // log.error('removePointer!', removePointer.$path, removePointer._$val)
        // if( removePointer._$val === null ) {
        //   log.shine('yes that purky is remove')
        //   var mphTarget = removePointer.$parent // << parent
        //   if(!mphTarget) {
        //     throw new Error(
        //       'could not find target to attach missing property listener!'
        //     )
        //   }
        ...
        //   )
        // } else {
        //   log.shine('do different.. nothing?')
        // }
...
function addMissingPropListener( target, emitter, pattern, originalTarget ) {
  ...
  log.shine('adding missing prophandler!', target && target.$path)
...
function createMissingPropHandler( target, emitter, pattern, originalTarget ) {
  return function missingPropHandler( event, meta ) {
    log.warn('********* missingPropHandler!', this.$path, meta)

    if(!meta) {
      return log.warn('meta is undefined?!', meta)
    }
...
function createAnyHandler( emitter, pattern, originalTarget ) {
  return function anyHandler( event, meta ) {
    ...
    log.warn('********* subscription property listener!', target.$path,
      '\n', pattern
    )
...
function addRemoveToMeta( meta, target, originalTarget, pattern ) {
  // get the "highest" removed property
  var pointer = getRemovePointer( target, pattern )
  if( pointer ) {
    target = pointer[0]
    pattern = pointer[1]
...
function walkRefs( target, emitter, pattern, originalTarget, skipFirst ) {
  var targetval = target._$val
  if( targetval instanceof Base ) {
    var firstRef = target
...
/**
 * Looks up the "highest" removed property, as seen from the originalTarget.
 * @param {Observable} target - removed Observable Starting point.
 * @param {Base} pattern - piece of the subscription pattern that holds the
 * information about references.
 */
function getRemovePointer( target, pattern ) {
  // console.log('figure out da remose from', target.$path)

  target = unWalkRefs( target, pattern )
  var pointer = [ target, pattern ]

  while( target._$val === null ) {
    // log.warn('ok remove!', target.$path)
    target = target.$parent
    pattern = pattern.$parent
    target = unWalkRefs( target, pattern )

    // log.warn('now look at', target.$path, target._$val)

    if( target._$val === null || target._$val instanceof Base ) {
      // log.warn('update pointer go deeper!')
      pointer = [ target, pattern ]
    }
  }
  //
  // var removed = unWalkRefs( target, pattern )
  // var pointer = [ removed, pattern ]
  // while(removed._$val === null) {
  //   pointer[0] = removed
  //   pointer[1] = pattern
  //   if( removed._$val === null ) {
  //     removed = removed.$parent
  //     pattern = pattern.$parent
  //     removed = unWalkRefs( removed, pattern )
  //   }
  // }

  // log.shine('jee removePointer', pointer[0].$path, pointer[1])

  return pointer
}

function unWalkRefs( target, pattern ) {
  var l = target.$key === 'reffed4'
  l && log.warn('hey unwalkrefs!', target.$path, pattern)

  var refsCache = pattern._refs
  var cached = refsCache && refsCache[target.$uid]
  l && log('cache?', pattern, refsCache, cached)

  if( cached ) {
    log.error('>> yes found dat reffer!', cached.$path)
    target = cached
  }
  l && log.warn('unwalked dat refs', target.$path)
  return target
}
