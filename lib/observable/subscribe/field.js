...
exports.define = {
  subField (data, event, obs, field, current, mapvalue, map) {
    ...
    if (typeof value === 'object') {
      return this.subObj(data, event, obs, value, current, mapvalue, map)
    }
    let id
    if (value === true) {
      id = this.generateId()
    ...
    if (id) {
      ...
      obs.on('data', [onData, this, field, mapvalue, map], this.key + id)
  ...
  subFieldRef (data, event, obs, field, current, mapvalue, map, context, contextmapvalue) {
    var value = field.val
    if (typeof value === 'object') {
      return this.subObj(data, event, obs, value, current, mapvalue, map, context, contextmapvalue)
    }
    let id
    ...
    if (id) {
      ...
      if (data) {
        ...
        this.findEmitRef(data, event, obs, mapvalue, map, context, contextmapvalue)
      }
      obs.on('data', [onDataRef, this, field, mapvalue, map, context, contextmapvalue], this.key + id)
      field.val = setId(current, id)
      // remove reference listeners
    }
