    ...
    if (id) {
      ...
      if (currentAttach && typeof mapvalue === 'object') {
        ...
        obs.on('remove', function(){
          ...
          emitter.findResubscribe(data, event, this, field, mapvalue)
        })
      }
  ...
  subFieldRef (data, event, obs, field, current, mapvalue, map, context) {
    ...
    if (id) {
      ...
      if (currentAttach && typeof mapvalue === 'object') {
        ...
        let emitter = this
        obs.on('remove', function (data) {
          console.error('remove ref!!',data)
          emitter.findResubscribe(data, event, context, field, mapvalue)
        })
      }
