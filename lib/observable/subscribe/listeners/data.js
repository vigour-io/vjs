...
module.exports = {
  direct (data, event, emitter, pattern, current, mapvalue) {
    console.log('--- data fires ---->', this.path.join('.'), emitter.path.join('.'))
  ...
  reference (data, event, emitter, pattern, current, mapvalue, context) {
    console.log('--- ref data fires ---->', this.path.join('.'))
    pattern.setKey('_stamp', this._lastStamp)

    if (data === null) {
      let patternContext = pattern._context
      patternContext = patternContext && patternContext._parent._parent
      let instances = patternContext && patternContext._instances
      if (instances) {
        let path = []
