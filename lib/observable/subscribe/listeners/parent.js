...
module.exports = {
  direct (data, event, emitter, pattern, current, mapvalue) {
    ...
    emitter.subField(true, event, this.parent, pattern.sub_parent, current, map)
  },
  reference (data, event, emitter, pattern, current, mapvalue, map, context) {
    event.fromParent = true
    emitter.subFieldRef(true, event, this.parent, pattern.sub_parent, current, mapvalue, map, context)
  }
}
