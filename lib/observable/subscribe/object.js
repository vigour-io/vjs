...
exports.define = {
  subObj (data, event, obs, pattern, current, mapvalue, map) {
        ...
        if (key === '$any') {
          ...
          if (!addedAnyListener) {
            ...
            if (!mergeListener(obs, 'property', anyid, mapvalue)) {
              obs.on('property', [onAny, this, pattern, current, mapvalue, map], anyid)
          ...
          if (property && !isRemoved(property)) {
            ...
            console.info('add parent listener:', obs.path.join('.'), JSON.stringify(pattern.serialize()))
            obs.on('parent', [onParent, this, pattern, current, mapvalue, map], id + '_parent')
          }
        } else {
          ...
          if (property && !isRemoved(property)) {
            ...
            if (!mergeListener(obs, 'property', propid, mapvalue)) {
              console.info('add prop listener:', obs.path.join('.'), JSON.stringify(pattern.serialize()))
              obs.on('property', [onProperty, this, pattern, current, mapvalue, map], propid)
              console.info('add remove listener:', obs.path.join('.'), JSON.stringify(pattern.serialize()))
              obs.on('remove', [onRemove, this, current, mapvalue], id)
            }
            addedListener = true
          }
    ...
    if (incomplete) {
      console.info('add ref listener:', obs.path.join('.'))
      obs.on('reference', [onReference, this, pattern, current, mapvalue, map], id + '_ref')
  ...
  subObjRef (data, event, obs, pattern, current, mapvalue, map, context) {
    ...
    for (let key in pattern) {
      ...
      if (key[0] === '_' || key === 'key') {
        ...
        if (!addedAnyListener) {
          let anyid = id + '_any'
          if (!mergeListener(obs, 'property', anyid, mapvalue)) {
            obs.on('property', [onAnyRef, this, pattern, current, mapvalue, map, context], anyid)
            obs.on('remove', [onRemoveRef, this, current, mapvalue, context], id + '_rm')
          }
          addedAnyListener = true
        }
        obs.each((property, key) => {
        ...
        if (property && !isRemoved(property)) {
          ...
          obs.on('parent', [onParentRef, this, pattern, current, mapvalue, map, context], id + '_parent')
        }
      } else {
        ...
        if (property && !isRemoved(property)) {
          ...
          let propid = id + '_prop'
          if (!mergeListener(obs, 'property', propid, mapvalue)) {
            obs.on('property', [onPropertyRef, this, pattern, current, mapvalue, map, context], propid)
            obs.on('remove', [onRemoveRef, this, current, mapvalue, context], id + '_rm')
          }
          addedListener = true
        }
    ...
    if (incomplete) {
      obs.on('reference', [onReferenceRef, this, pattern, current, mapvalue, map, context], id + '_ref')
      let ref = reference(obs)
      if (ref) {
        this.onSubHandler(event, pattern, ref, obs)
