...
exports.define = {
  subObj (data, event, obs, pattern, current, mapvalue, map) {
    ...
    console.log('----add reference listener', obs.path)
  ...
  subObjRef (data, event, obs, pattern, current, mapvalue, map, context) {
    ...
    pattern.each((field, key) => {
      ...
      if (key === 'upward') {
        ...
        if (!isRemoved(property)) {
          // map[obs.key] = mapvalue
          found = this.subFieldRef(data, event, property, field, current, mapvalue, context)
        } else {
          obs.on('parent', [onParentRef, this, pattern, current, mapvalue, map, context], this.key)
        }
      } else {
        let property = obs[key]
        if (property && !isRemoved(property)) {
          ...
          obs.on('property', [onPropertyRef, this, pattern, current, mapvalue, map, context], this.key)
          addedListener = true
        }
      }
    ...
    console.log('-----add reference listener REF', obs.path, this.key)

    obs.on('reference', [onReferenceRef, this, pattern, current, mapvalue, map, context], this.key)
    let ref = reference(obs)
    if (ref) {
      return this.subObjRef(data, event, ref, pattern, incrementLevel(current), mapvalue, map, obs)
    }
