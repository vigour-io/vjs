...
var mergeObj = mergeListener.obj
...
exports.define = {
  subObj (data, event, obs, pattern, current, mapvalue, map) {
    ...
    console.info('- ',obs.path, JSON.stringify(mapvalue))
        ...
        if (key === '$any') {
          obs.each((property, key) => {
            ...
            if (property && !isRemoved(property)) {
              console.log('ANY', key, JSON.stringify(mapvalue))
              map.parent = mapvalue
          ...
          this.subCondition(data, event, obs, pattern, current, mapvalue)
        } else if (key === '$upward') {
          ...
          console.log('checking - ', obs.key,  ' - ',key)
          if (property && !isRemoved(property)) {
            console.log('gogogo', obs.key,  ' - ',key)
            map.parent = mapvalue
            found = this.subField(data, event, property, pattern[key], current, map)
          } else if (!addedListener) {
            // console.log('---add prop listener:',JSON.stringify(mapvalue, false, 2), obs.path)
            if (!mergeListener(obs, 'property', id, mapvalue)) {
              // console.log(' no merge just add')
