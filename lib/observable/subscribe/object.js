...
exports = {
  ...
  subObjRef (data, event, obs, pattern, current, mapvalue, map) {
    var addedListener, incomplete
    pattern.each((field, key) => {
      var found
      if (key === 'upward') {
        found = this.subUpRef(data, event, obs, field, current, mapvalue, map)
      } else if (key === 'sub_parent') {
        let property = obs.parent
        if (!isRemoved(property)) {
          map[obs.key] = mapvalue
          found = this.subFieldRef(data, event, property, field, current, map)
        } else {
          obs.on('parent', [onParentRef, this, pattern, current, mapvalue, map], this.key)
        }
      } else {
        let property = obs[key]
        if (!isRemoved(property)) {
          ...
          obs.on('property', [onPropertyRef, this, pattern, current, mapvalue, map], this.key)
          addedListener = true
        }
      }
    ...
    obs.on('reference', [onReferenceRef, this, pattern, current, mapvalue, map], this.key)
    let ref = reference(obs)
    if (ref) {
      return this.subObjRef(data, event, ref, pattern, incrementLevel(current), mapvalue, map)
    }
