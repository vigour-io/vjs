...
exports.define = {
    ...
    for (let key in pattern) {
      ...
      if (key[0] === '_' || key === 'key') {
        ...
        if (key === '$any') {
          ...
          if (property && !isRemoved(property)) {
            console.log('property:'.blue, obs.path.join('.'), key)
      ...
      if (ref) {
        if (ref.onSub) {
          if (this._fromUpward !== obs && !pattern._ignore) {
            ...
            ref.onSub(data, event, this, pattern, current, mapvalue, map, obs)
          }
        }
  ...
  subObjRef (data, event, obs, pattern, current, mapvalue, map, context) {
    ...
    for (let key in pattern) {
      ...
      if (key[0] === '_' || key === 'key') {
        ...
        if (!addedAnyListener) {
          let id = id + 'Any'
          if (!mergeListener(obs, 'property', id, mapvalue)) {
            obs.on('property', [onAnyRef, this, pattern, current, mapvalue, map, context], id + 'Any')
          }
          addedAnyListener = true
        }
        obs.each((property, key) => {
    ...
    if (incomplete) {
      ...
      if (ref) {
        if (ref.onSub) {
          console.info('maybe do?')
          if (this._fromUpward !== obs && !pattern._ignore) {
            console.info('DO SOMETHING', pattern._ignore)
            let onRemove = this.onRemove
            if (onRemove) {
              onRemove.push(ref)
            } else {
