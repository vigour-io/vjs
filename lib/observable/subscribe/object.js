...
exports.define = {
  subObj (data, event, obs, pattern, current, mapvalue, map) {
    var id = this.uid + '.' + pattern.uid + '.dir'
        ...
        if (key === '$any') {
          ...
          if (property && !isRemoved(property)) {
            ...
            if (!mergeListener(obs, 'property', id, mapvalue)) {
              obs.on('property', [onProperty, this, pattern, current, mapvalue, map], id + '_prop')
              obs.on('remove', [onRemove, this, current, mapvalue], id)
            }
            addedListener = true
          }
  ...
  subObjRef (data, event, obs, pattern, current, mapvalue, map, context) {
    var id = this.uid + '.' + pattern.uid + '.ref'
    var addedListener, incomplete
    for (let key in pattern) {
      let found
      if (key[0] === '_' || key === 'key') {
        ...
        if (property && !isRemoved(property)) {
          ...
          if (!mergeListener(obs, 'property', id, mapvalue)) {
            obs.on('property', [onPropertyRef, this, pattern, current, mapvalue, map, context], id + '_prop')
            obs.on('remove', [onRemoveRef, this, current, mapvalue, context], id + '_rm')
          }
          addedListener = true
        }
