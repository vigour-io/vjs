...
exports.define = {
  subUp (data, event, obs, pattern, current, mapvalue, map) {
    ...
    if (parent) {
      ...
      console.log('-----upward map',JSON.stringify(map))
      return this.subUp(data, event, parent, pattern, current, map, {})
    }

    // let currentAttach = obs._on
    //   && obs._on.parentEmitter
    //   && obs._on.parentEmitter.attach
    //   && obs._on.parentEmitter.attach[this.uid + 'u']

    // if (currentAttach && typeof mapvalue === 'object') {
    //   merge(currentAttach[4], mapvalue)
    // }else{
      console.log('adding it', obs.path, JSON.stringify(mapvalue), JSON.stringify(map))
      obs.on('parent', [onUp, this, pattern, current, mapvalue, map], this.uid + 'u')
    // }
  },
  subUpRef (data, event, obs, pattern, current, mapvalue, map, context) {
    ...
    obs.on('parent', [onUpRef, this, pattern, current, mapvalue, map, context], this.uid + 'u')
  }
}
