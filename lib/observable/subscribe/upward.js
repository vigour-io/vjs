...
exports = {
  ...
  subUpRef (data, event, obs, pattern, current, mapvalue, map) {
    if (this.subObjRef(data, event, obs, pattern, current, mapvalue, map)) {
      return true
    }
    let parent = obs.parent
    if (parent) {
      map[obs.key] = mapvalue
      return this.subUpRef(data, event, parent, pattern, current, map, {})
    }
    obs.on('parent', [onUpRef, this, pattern, current, mapvalue, map])
  }
}
