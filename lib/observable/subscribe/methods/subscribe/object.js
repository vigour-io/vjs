...
module.exports = function subscribeObject (data, event, obj, pattern, info, mapValue, mapObj) {
  ...
  if (patternAny) {
    ...
    pattern.each((patternProperty, key) => {
      var found
      if (key === 'upward') {
        found = this.subscribeUpward(data, event, obj, patternProperty, info, mapValue, mapObj)
      } else if (key === '&') {
        // subcribeDeep
        ...
        if (objProperty && objProperty._input !== null) {
          ...
          console.error('---->adding prop')

          if(data.context){
            this.addSubListener(obj, 'property', [onProperty, this, pattern, info, mapValue, mapObj, data])
          }else{
