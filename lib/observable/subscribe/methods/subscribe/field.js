...
module.exports = function subscribeField(data, event, obj, pattern, key, info, mapValue) {
  ...
  console.error('field',key,id, info, getLateral(info), getDepth(info), obj.path, data.context)
  ...
  if (id || data) {
    ...
    if (data.context) {
      ...
      console.error('do it',subscriber.path, this.parent.parent.path, subscriber._context)
