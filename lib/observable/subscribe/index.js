...
var SubsEmitter = new Emitter( {
  ...
  $define: {
    $addChangeListener: function( property, val, key, event, refLevel, level, meta ) {
      ...
      property.on( $CHANGE, [ onChange, this, refLevel, level ], propPath )
      
      //if this came from a listener, fire the subscription
      if( meta ) {
        console.log('EMITTING!!!',this.$parent.$parent,val)
        this.$parent.$parent.emit(this.$key)
      }
    },
    $subscribeUpward: function( obj, val, event, refLevel, level, meta ) {
      ...
      if( !fulfilled ) {
        ...
        console.error('GVD',obj)

        obj.on( $ADDTOPARENT, function(){
          console.error('THIS IS WERID')
        })
...
function onChange( event, meta, subsemitter, refLevel, level ) {
  ...
  // //finds correct parent
  var parent = this.$parent
  var key = subsemitter.$key
  var on

  while(parent){
    on = parent.$on
    if(on && on[key] === subsemitter){
      break
    }
    parent = parent.$parent
  }

  subsemitter.emit(event, parent)
