...
module.exports = function subscribe( target, emitter, pattern,passon, originalTarget, event ) {
  ...
  console.warn('???????', emitter, emitter.$pattern)
...
function addListeners(target, listeners, pattern) {
  for(var type in listeners) {
    console.log('ADD SUBSCRIPTION LISTENERS', target.$path)
    target.on(type, [listeners[type], pattern], false, 
      function subsCompare( compare ) {
        return compare[1] !== pattern
      }
    )
  }
}
      ...
      for(var i = 0, l = added.length ; i < l ; i++) {
        ...
        if(firstChar !== '_' && firstChar !== '$') {
          ...
          if(sub.$val === true) {
            ...
            targetProp._subscribe(sub, listeners, event)
          } 
        }
      }
    }
