    ...
    remove (event, nocontext, noparent, trigger) {
      ...
      if (event) {
        ...
        if (trigger) {
          ...
          // let stored
          // if (this._context) {
          //   console.log('resolve it!') best to resolve it immediatly much better
          //   make extra tests for this but should not work! without
          //   //resolve
          //   // stored = this.storeContextChain()
          // }
          event.on('close', () => {
            this.removeFromInstances(event)
            _remove.call(this, false, nocontext, noparent)
          })
          return this
        }
      } else {
