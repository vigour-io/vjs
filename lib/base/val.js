...
define( proto, '$parseValue', {
  value:function( previousValue, origin ) {
    ...
    if(this._$cachedStamp && this._$cachedStamp === this._$lastChange ) {
      ...
      //     if(this[i] && this[i]._$cachedStamp!==this._$cachedStamp) {
      //       // skipit = true
      //       break
      //     }
      //   }
      ...
      if( operator.$hasOperators( this ) ) {
        ...
        for( var key$ in this ) {
          if(operators[key$]) {
            ...
            console.info('exec operator', operator$.$path, operator$._$lastChange)
            

            // operator$._$lastChange = this._$lastChange

            val = operator$._$operator.call( this, val, operator$, origin )
            // operator._$cached = val
            console.info('----?',operator$.$path, operator$._cached)
            // if(!operator$._$lastChange) {
              // console.info('hello!', this._$lastChange)
            // }
          }
        }
      }
