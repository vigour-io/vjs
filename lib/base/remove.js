...
exports.removeUpdateParent = function (parent, event, context) {
  if (parent[this.key] === null) {
    console.warn('allready removed!', this.key)
    return true
  }
  var key = this.key
  if (context) {
    if (this._context) {
      console.error('ok ok context!')
...
exports.removeProperties = function (event, nocontext) {
  for (var key in this) {
    if (key === '_parent') {
      ...
      this.removeProperty(this[key], key, false, nocontext)
    }
  }
  // this.parent = null
  this._input = null
...
exports.remove = function (event, nocontext, noparent) {
  console.warn('\n\nreal remove', this._context, this.path, nocontext)
