...
exports.storeContextChain = function () {
  ...
  contextUp = context
  while (contextUp) {
    if (contextUp._context) {
      chain.push({
        context: contextUp._context,
 ...
 */
exports.contextCandidate = function (key, value) {
  if (!value) {
    value = this[key]
  }
  // console.log('try:', key)
  ...
  if (value && value.createContextGetter) {
    var hasUnderscore = key[0] === '_'
    var overide = this.get(['_properties', key, 'overide'])
    if( overide ) {
      console.warn(overide)
    }
    if (hasUnderscore &&
      (this._properties) &&
      this._properties[key]
    ) {
      // console.log('???', this._properties[key].overide)
      if (this._properties[key].overide && this._properties[key].overide === key) {
        hasUnderscore = false
      }
    }
    if (!hasUnderscore && !this['_' + key]) {
      // console.error(value)
      return value
    }
  }
