      ...
      for (var j in this[i]) {
        if (j !== 'val') {
          if (this[i][j].path.indexOf('_on') < 0) {
            ...
            str += '\n             ----> ' + this[i][j].path + '  uid:' + this[i].val.uid
          }
        }
      }
    }
...
exports.resolveContext = function (val, event, context) {
  ...
  console.log('FUCK FACE', iterator._path, context._path)

  var proto = Object.getPrototypeOf(context)
  while (iterator && iterator !== context && iterator !== proto) {
    // console.log(iterator === context, context._path, 'vs', iterator._path)
    // if (iterator.key === 'BITCH') {
      // console.log('yo lezzgo', iterator, 'vs', context)
    // }
