...
exports.$resolveContextSet = function( val, event, context ) {
  ...
  if(event) {
    event.$block = true
    event.$resolvingContext = context
  }
  //nu zorgen dat de context de resolved pakt en niet de nested resolves
  ...
  //   obj = obj[path[i]] = i === pathLengthCompare ? val : {}
  // }

  // context.set( setObj, event )

  // var passon = event
  // if(context._$instances) {
    // console.error('!@#@!#?')
    // passon = false
  // }
  // this.$clearContextUp()
  ...
  for( i = 0; i < pathLength; i++ ) {
    if(context) {
      ...
      context[path[i]],
      i === pathLengthCompare ? event : false, //false, //event, //so weird!
      true )
    }
  }
  console.groupEnd()
  ...
  if (resolveEventOrigin) {
    console.log('?resolve?', event.$postponed)
    event.$context = null
    // event.$resolvedContext = context
  ...
  get:function() {
    if(this._$contextLevel) {
      if(this._$contextLevel === 1) {
        console.log('resolve? it', this._$path, this._$context._$context && this._$context._$context._$path)
