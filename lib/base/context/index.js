...
exports.resolveContext = function (val, event, context) {
  ...
  if (context._contextKey) {
    console.log('context', context._contextKey)
  }

  if (context._context) {
    console.warn('how to handle this?')
    context = context.resolveContext({}, event, context._context)
  ...
  for (i = 0; i < pathLength; i++) {
    if (context) {
      ...
      let prop = context[path[i]]

      let xx
      // console.log('ok ok resolve!', path[i], this._contextKey)
      ...
      context = context.setKeyInternal(
        path[i],
        set,
        i === pathLengthCompare ? prop || this : prop,
        event,
