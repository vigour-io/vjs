...
//this can be cleaned upa little bit

function setKeyFlagHelper( Constructor, key, val, privateField, event ) {
  var field = this[ privateField ]
  if(field) {
    this.$setKeyInternal( privateField, new Constructor(val, event), field )
  } else {
...
function flagConstructorHelper(Constructor, key) {
  ...
  return function constructorHelper(val, event) {
    setKeyFlagHelper.call( this, Constructor, key, val, privatefield, event )
  }
}
