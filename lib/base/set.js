...
exports.addNewProperty = function (key, val, property, event, escape) {
  ...
  this[key] = val
  var parent = this
  console.log('ok lets resolve some shit', key)
  console.log('????')
  while (parent) {
    // situation
    /*
      var x = {x:{}}
      var b = new x(x:{  b })
      // b should get a context getter in this case?
      // or context getters need to be applied to nested fields of instances
      // yes thats it -- need to check if a prop is its own then check if it has its own prop constructor
    */
