...
define( proto, '$setValue' , {
  value: function(val, event) {
    if(val instanceof Base) {
      ...
      //no add listens on val
      //do need to check if there is multiple
      //hard part is that it needs to check if there 
      //is another 
...
define( proto, '$setKeyInternal', {
  value:function( key, val, property, event ) {
    if(property) {
      ...
      //losse functie in on .$postpone also does all the checks
      

      //!!!maak emit types!!! voor global
      if(event) {
        var propertyEmitter = this.$on && this.$on.$property
        if( propertyEmitter ) {
          var stamp = event.$stamp
          if( this.$propertyStamp !== stamp) {
            propertyEmitter._$meta = {
              added:[ key ]
            }
            this.$propertyStamp = stamp
            event.$postpone(propertyEmitter)
          } else {
            propertyEmitter._$meta.added.push( key )
          }
        }
      }
      //dit is nog niet clean
