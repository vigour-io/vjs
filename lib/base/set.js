  ...
  if (isPlainObj(val)) {
    if (resolveContext) {
      ...
      for (let key in val) {
        ...
        if (key === 'val') {
            console.log('1 yo biatch HERE! setKeyInternal is doin it')
          if (base.setValue(val[key], event, resolveContext)) {
            changed = true
          }
        } else {
          console.log('2 yo biatch HERE! setKeyInternal is doin it')
    ...
    if (property._parent !== this) {
      if (val === null) {
        ...
        if (escape && !Constructor) {
          ...
          console.error('problems?', this._context, key)
          // why is this not handled by resolve context???????
...
exports.setKey = function (key, val, event, nocontext, escape) {
  ...
  console.log('yo biatch HERE! setKeyInternal is doin it')
