...
exports.set = function( val, event, nocontext ) {
  ...
  if(util.isPlainObj(val)) {
    if( !nocontext && this._$context ) {
      ...
      if(!changed) {
        //should just remove itself from the event but still clear it!
        return true
      }
    }
  } else {
...
exports.$setKeyInternal = function( key, val, property, event, nocontext ) {
  if( property) {
    if( property._$parent !== this ) {
      ...
      //but now the situation can be there where the event is part of this
      //(not property) and then it can happen that i does not complete the event!
      //set does not need to block it just needs to know that it does not need to exec itself
      // }
      // return true
    }
  } else {
