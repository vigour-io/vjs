...
exports.set = function (val, event, nocontext, escape) {
  ...
  if (isPlainObj(val)) {
    // do something with type prob smart
...
exports.setKeyInternal = function (key, val, property, event, nocontext, escape) {
  if (property) {
    ...
    if (property._parent !== this && !property.noInstance) {
      ...
      // ** NEEDS TESTS **
      // if (property.noInstance) {
      //   // console.log('ok lets do it!', val, property._path)
      // }
...
exports.addNewProperty = function (key, val, property, event, escape) {
  ...
  var parent = this
  while (parent) {
    // console.log('YO YO YO YO', key, parent.key)
...
exports.setKey = function (key, val, event, nocontext, escape) {
  var type = key
  console.log('set key', key, this, val)
  ...
  if (isPlainObj(val) && val.type && this.getType) {
    val = this.getType(val, event, key, nocontext, escape)
    console.log(val)
  }
