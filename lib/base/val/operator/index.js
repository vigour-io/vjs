...
var Operator = module.exports = new Base({
  ...
  $define: {
    ...
    $makeResults: function(val, copy) {
      var operator = this
      var isList = val instanceof List
      var Type = isList ? List : Base
      var results = operator._results = new Type() 
      // TODO: fix results.$parent
      results._$key = '$results'
      if(copy && val) {
        if(isList) {
          // TODO: make faster way to copy all fields
          if(val) {
            if(util.isPlainObj(val)) {
              forEach(val, function(value, key ) {
                results.$push( value )
