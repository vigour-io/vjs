...
exports.$resolveContextSet = function( val, event ) {
  ...
  var i = path.length-this._$contextLevel-1
  ...
  if(event) {
    //this is ofc very wrong try to fix without event
    //its just that you want to pass it to all sets
    
    //change constructor
    //change setkeyInternal
    //change setKey
    //change remove

    event.$resolvingContext = true
    event.$block = true
    notchanged = parent.$set( setObj, event, true )
    event.$block = null
    event.$resolvingContext = null
  } else {
