...
//generate id will be the name of this method
...
function add( base, type, val, key ) {
  ...
  if(!base[type] ) { //|| base[type]
    //why does this not inherit correctly?
    //should merge :/
  ...
  for(var i in base[type]) {
    //bec arefull /w this may not be instersting for functions
    if(base[type][i]===val) {
  ...
  //handle arrays for types.... this will screw things up
  val.$useVal = true
  //will not work ofcourse

  base[type].$setKeyInternal( key, val, false )

  //not called here
  //should use a setInterna;
