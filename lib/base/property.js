 ...
 * @param {string} [overide] - overide the normal key
...
exports.propertyTypes = function (properties, property, key, val, event, nocontext) {
  ...
  if (prototype && prototype instanceof Base) {
    ...
    custom(properties, property, key)
  } else {
...
function custom (properties, property, key) {
  if (property.val && property.overide) {
    property.val = parseProperty(property.val)
    var prototype = property.val.prototype
    if (prototype && prototype instanceof Base) {
      properties[key] = Properties
        .createPropertyConstructor(property.val, property.overide)
      properties[key].overide = property.overide
      properties[property.overide] = properties[key]
    } else {
      propertyTypeError(property)
    }
  } else {
