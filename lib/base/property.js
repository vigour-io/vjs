...
function custom (properties, property, key, event) {
  if (property.val) {
    ...
    // console.warn('experimental - use child constructor')
    // console.error('yo may need to handle type as well right hur!', key)

    if (property.type && this.getType) {
      let res = this.getType(property, event, key, void 0)
      console.warn(property, key)
