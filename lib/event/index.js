'use strict'
var base = require('../base').prototype
// var timestamp = require('monotonic-timestamp')
var debug = require('../util/debug')
/**
 * @namespace Event
 * Constructor of Event
 * Events are used to bundle emitters and handle updates correctly
 * @param {base} [origin] origin base of event
 * @param {string} [type] type of event e.g "data"
 */
var cnt = 0
var Event = module.exports = function Event (type, stamp) {
  this.stamp = stamp || (cnt++)
  console.log('MAKE NEW EVENT', this.stamp, debug.stack().toString())
  if (type) {
    this.type = type
  }
}

var event = Event.prototype
// event.uuid = uuid
/**
 * @function define
 * same define as on base
 * @memberOf Event#
 */
Object.defineProperty(event, 'define', {
  value: base.define,
  configurable: true
})

event.define({
  /**
   * @function inject
   * same inject as on base
   * @memberOf Event#
   */
  inject: base.inject,
  on (type, val) {
    return this._on[type](this, val)
  },
  _on: {
    value: {
      close (event, val) {
        var onClose = event._onClose
        if (!onClose) {
          event._onClose = onClose = []
        }
        onClose.push(val)
        return event
      }
    }
  },
  /**
   * @function push
   * push emitters to the event queue
   * @param {emitter} [emitter] push an emitter to the event queue
   * @memberOf Event#
   * @returns {event}
   * @todo define secondary here and not in the loop
   */
  push (emitter, bind, data) {
    var target = this
    if (this._triggered) {
      if (this._triggered.stamp) {
        this._triggered.push(emitter, bind, data)
      } else {
        console.warn('NESTED EVENT lets make event', key, event, this._path)
        target = this._triggered = new Event(this.type)
        target.data = this.data
        target.stamp = this.stamp
      }
    }
    // this is where we store context restore paths!
    // and specific targets
    // and data per bind
    let uid = emitter.uid // make this faster emitter allways gets _uid
    if (!target[uid]) {
      target[uid] = {
        val: emitter,
        [bind.uid]: bind
      }
    } else if (!target[uid][bind.uid]) {
      target[uid][bind.uid] = bind
    }
  },
  trigger: require('./trigger.js'),
  /**
   * @function remove
   * clears event, removes props, removes references
   * @memberOf Event#
   */
  remove () { // rename this to close
    var onclose = this._onClose
    if (onclose) {
      let len = onclose.length
      for (let i = 0; i < len; i++) {
        onclose[i](this)
      }
      this._onClose = null
    }
    if (this._triggered && this._triggered.trigger) {
      this._triggered.trigger()
    }
    // this.origin = null
    // this.resolving = null
    // this.inherited = null
    // this.removed = true
  }
})
