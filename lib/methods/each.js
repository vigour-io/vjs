 ...
 */
exports.define = {
  each: function (fn, filter, attach) {
    var length = this.length
    if (length) {
      if (filter) {
        eachArrayLikeFiltered(this, length, fn, filter, attach)
      } else {
        eachArrayLike(this, length, fn, attach)
      }
    } else {
      if (filter) {
        eachFiltered(this, fn, filter, attach)
      } else {
...
function eachArrayLikeFiltered (target, length, fn, filter, attach) {
  for (var i = 0; i < length; i++) {
    ...
    if (filter(iteratee, i, target, attach) && fn(iteratee, i, target, attach)) {
      return
    }
  }
}
...
function eachFiltered (target, fn, filter, attach) {
  console.log('hey filtwer!', filter)
  for (var i in target) {
    var iteratee = target[i]
    if (i[0] !== '_' &&
      iteratee !== null &&
