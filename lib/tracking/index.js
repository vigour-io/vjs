...
function addListenerKey (observable, key, event) {
  observable.on(key, function (data, event) {
    trackingEmitter.emit(data, event, observable, key)
  }, 'track')
}

function arrayTracking (id, event) {
  var el = id.length
  var observable = this
  for (var i = 0; i < el; i++) {
    addListenerKey(observable, id[i], event)
  }
}
